...
To match the policy budget and the numerical outputs, we measure investment in \emph{hundred-million RMB} (亿元).
Thus, the total budget is
\begin{equation}
\sum_{j=1}^{p} I_j = B,\qquad B=10000\ \text{(亿元)}.
\label{eq:t4_budget}
\end{equation}
...
All allocations below are in \emph{亿元}.
...
\begin{tabular}{rlrr}
\toprule
Rank & Indicator & Investment (亿元) & Share (\%) \\
\midrule
